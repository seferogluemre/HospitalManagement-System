...
import type { TreatmentCreatePayload, TreatmentUpdatePayload } from "../types";

export abstract class TreatmentService {
  static async createWithAI(
  ...
  static async regenerateAIReport(uuid: string) {
    try {
      ...
      const updatedTreatment = await BaseTreatmentService.update(
        uuid,
        updateData
      );
